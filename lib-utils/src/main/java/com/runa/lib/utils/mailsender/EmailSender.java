package com.runa.lib.utils.mailsender;
...
import com.runa.lib.api.utils.IEmailSender;
...
public class EmailSender implements IEmailSender {
	...
	public void sendEmailsFromAdmin(BookDto dto) throws MessagingException {
		...
		MimeMessageHelper helper = new MimeMessageHelper(message);
		String text = prepareActivateRequestEmail(dto);
		for (User user : bookConverter.convertStringToListDepartments(dto.getDepartment()).get(0).getUsers()) {
	...
	private VelocityContext createVelocityContextWithBasicParameters(BookDto dto) {
		...
		context.put("name", dto.getBookDetails().getName());
